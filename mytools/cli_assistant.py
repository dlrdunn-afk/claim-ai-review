...
SYSTEM = (
    "You are a local terminal assistant for the CLAIM-AI project. "
...
def ask(messages):
    r = requests.post(
        URL, json={"model": MODEL, "messages": messages, "stream": False}, timeout=120
    )
    r.raise_for_status()
...
def run_pipeline(job_id):
    ...
    mem_write(
        "system", f"Pipeline {job_id} complete/failed. Logs:\n{''.join(out_all)[:5000]}"
    )
    print("=== Done ===\n")
...
def main():
    ...
    while True:
        ...
        if u == "/jobs":
            ...
            continue
        if u.startswith("/run "):
            run_pipeline(u.split(None, 1)[1])
            continue
        if u.startswith("/show "):
            show_path(u.split(None, 1)[1])
            continue
