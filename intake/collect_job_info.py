...
if cause == "flood":
    ...
    has_photos = (
        input("Were photos taken before mitigation? (yes/no): ").strip().lower()
    )
    metadata["pre_mitigation_photos"] = has_photos == "yes"

elif cause == "fire":
    fire_type = (
        input("Was it a kitchen fire, electrical, lightning, or unknown?: ")
        .strip()
        .lower()
    )
    ...
    interior_damage = (
        input("Was there interior water damage? (yes/no): ").strip().lower()
    )
